...
alter table reservation add constraint fk_reservation_user_8 foreign key (user_id) references user (id) on delete restrict on update restrict;
create index ix_reservation_user_8 on reservation (user_id);
alter table reservation add constraint fk_reservation_place_9 foreign key (place_id) references place (id) on delete restrict on update restrict;
create index ix_reservation_place_9 on reservation (place_id);
alter table reservation add constraint fk_reservation_status_10 foreign key (status_id) references status (id) on delete restrict on update restrict;
create index ix_reservation_status_10 on reservation (status_id);
alter table user add constraint fk_user_avatar_11 foreign key (avatar_id) references image (id) on delete restrict on update restrict;
create index ix_user_avatar_11 on user (avatar_id);
